"""Core orchestrator: middleware → router → inference → telemetry."""
...
from my_routing_agent.routers.engine import RouteDecision, RoutingEngine, RoutingResult
...
class HybridRoutingAgent:
    ...
    def _invoke_with_fallback(
        ...
        *,
        ...
        fallback = (
            ...
            if primary == RouteDecision.LOCAL
            else RouteDecision.LOCAL
        )
        ...
        print(
            f"[fallback] Primary destination '{primary.value}' failed: {response.error}. "
            f"Retrying via '{fallback.value}'.",
            file=sys.stderr,
        )
        fallback_response = self._dispatch(
            fallback, messages, json_mode=json_mode, schema=schema
        )
    ...
    def _dispatch(
        ...
        *,
        ...
        remote_response = self.remote_client.chat(
            messages,
            json_mode=json_mode or True,
            schema=schema,
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Hybrid token-efficient routing agent (local Ollama ↔ Fireworks AI)."
    )
    parser.add_argument(
        "prompt",
        nargs="?",
        default="What is 17 * 23?",
        help="User task / prompt text.",
    )
    parser.add_argument(
        "--image",
        action="append",
        default=[],
        dest="images",
        help="Optional image path(s) for multimodal routing.",
    )
    parser.add_argument(
        "--json",
        action="store_true",
        help="Request structured JSON output (strict on remote).",
    )
    parser.add_argument(
        "--force-local",
        action="store_true",
        help="Bypass router and force local inference.",
    )
    parser.add_argument(
        "--force-remote",
        action="store_true",
        help="Bypass router and force remote inference.",
    )
